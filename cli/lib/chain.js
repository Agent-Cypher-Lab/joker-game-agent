...
function init({ rpcUrls, rpcUrl, jokerGameAddress, tokenAddress, identityAddress }) {
...
function _rebindContracts(idx) {
  ...
  _token = new ethers.Contract(_tokenAddr, ERC20_ABI, _providers[idx])
}
...
async function getAllowance(ownerAddress) {
  return _token.allowance(ownerAddress, _gameAddr)
}

async function getTokenBalance(ownerAddress) {
  return _token.balanceOf(ownerAddress)
}
